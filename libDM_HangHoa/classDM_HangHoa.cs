        ...
        public List<DM_TheKhoDTO> GetListTheKho(Guid id, Guid iddonvi)
        {
            ...
            if (db != null)
            {
                ...
                lst = tbl1.AsEnumerable().GroupBy(p => p.ID_HoaDon).Select(t => new DM_TheKhoDTO()
                {
                    ...
                    LoaiChungTu = t.FirstOrDefault().LoaiHoaDon == 1 ? "Bán hàng" : (t.FirstOrDefault().LoaiHoaDon == 4 ? "Nhập hàng" : (t.FirstOrDefault().LoaiHoaDon == 10 && t.FirstOrDefault().YeuCau == "1" || (t.FirstOrDefault().YeuCau == "4" && t.FirstOrDefault().ID_DonVi == iddonvi) ? "Chuyển hàng" : (t.FirstOrDefault().LoaiHoaDon == 9 ? "Kiểm hàng" : (t.FirstOrDefault().LoaiHoaDon == 8 ? "Xuất kho" : (t.FirstOrDefault().LoaiHoaDon == 6 ? "Trả hàng" : (t.FirstOrDefault().LoaiHoaDon == 18 ? "Điều chỉnh giá vốn" : (t.FirstOrDefault().LoaiHoaDon == 10 && t.FirstOrDefault().YeuCau == "4" && t.FirstOrDefault().ID_CheckIn == iddonvi ? "Nhận chuyển hàng" : "Trả hàng nhập"))))))),

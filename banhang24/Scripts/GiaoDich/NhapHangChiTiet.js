...
var NhapHangChiTiet = function () {
    ...
    function UpdateHoanTra_ifUndefined() {
    ...
    function CheckSaoChep_EditPhieuNhap1() {
        var isSaoChep = localStorage.getItem('isSaoChep') === 'true';
        var isEdit = localStorage.getItem('isEditNH') === 'true';
        if (isSaoChep || isEdit) {
            var cthd = localStorage.getItem('lc_CTSaoChep');
            if (cthd !== null) {
                cthd = JSON.parse(cthd);
                ...
                console.log('objHD ', objHD)

                $('#selectedNV').val(cthd[0].ID_NhanVien);
                self.newHoaDon().SetData(objHD[0]);
                localStorage.setItem(lcHDNhapHang, JSON.stringify(objHD));
                localStorage.setItem(lcCTNhapHang, JSON.stringify(cthd));
            ...
            if (typeCacheNhapHang !== null) {
                ...
                if (cthd !== null) {
                    ...
                    console.log('objHD ', objHD)
                    $('#selectedNV').val(objHD[0].ID_NhanVien);
                    self.newHoaDon().SetData(objHD[0]);
                    localStorage.setItem(lcHDNhapHang, JSON.stringify(objHD));
                    localStorage.setItem(lcCTNhapHang, JSON.stringify(cthd));
                    self.HangHoaAfterAdd(cthd);
    ...
    function CheckSaoChep_EditPhieuNhap() {
        ...
        if (cthd !== null) {
            ...
            $('#selectedNV').val(objHD.ID_NhanVien);
            localStorage.removeItem('isSaoChep');
            localStorage.removeItem('isEditNH');
            localStorage.removeItem('lc_CTSaoChep');
            localStorage.removeItem('typeCacheNhapHang');
            ...
            if (cthd.length > 0) {
                dialogConfirm_OKCancel('Thông báo', 'Hệ thống tìm được 1 bản nháp chưa được lưu lên máy chủ. Bạn có muốn tiếp tục làm việc với bản nháp này?', function () {
                    ...
                    if (hd !== null) {
                        ...
                        $('#selectedNV').val(hdLast.ID_NhanVien);
                    }
                }, function () {
                    RemoveCache();
                    ResetInforHD();
    ...
    function newCTNhap(addCungLoai, itemHH, soluong) {
        ...
        let tienCK = ptCKHangHoa * itemHH.DonGia / 100;
        let tienThue = ptThue * (itemHH.DonGia - tienCK) / 100;
        ...
        if (addCungLoai) {
            return {
                ...
                ThanhTien: itemHH.DonGia - (ptCKHangHoa * itemHH.DonGia) / 100,// sauCK
                ThanhToan: itemHH.DonGia - tienCK + tienThue,// sauthue
                GhiChu: '',
            ...
            for (let i = 0; i < hd.length; i++) {
                if (hd[i].IDRandom === self.newHoaDon().IDRandom()) {
                    if (!commonStatisJs.CheckNull(hd[i].ID) && hd[i].ID !== const_GuidEmpty) {
                        hd[i].KhachDaTra = 0;
                        hd[i].HoanTra = 0;
                        hd[i].PhaiThanhToan = hd[i].TongTienHang + hd[i].TongTienThue - hd[i].TongGiamGia;

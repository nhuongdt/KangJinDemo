...
namespace banhang24.Areas.DanhMuc.Controllers
{
    public class BH_HoaDonAPIController : BaseApiController
    {
        ...
        public string Export_PhieuTrichHoaHong_byID([FromBody] JObject data)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                try
                {
                    ...
                    {
                        lstCell = data["lstCell"].ToObject<List<ClassExcel_CellData>>();
                    }
        ...
        [HttpPost]
        public void ExportExcel_HoaDonBanLe(Params_GetListHoaDon listParams)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ClassBH_HoaDon classhoadon = new ClassBH_HoaDon(db);
                Class_officeDocument _classOFDCM = new Class_officeDocument(db);
                ClassNPOIExcel classNPOIExcel = new ClassNPOIExcel();
                string fileSave = string.Empty;
                try
                {
                    List<BH_HoaDonDTO> lstAllHDs = classhoadon.GetListInvoice_Paging(listParams);
        ...
        public string ExportExcel_GoiDichVu(Params_GetListHoaDon listParams)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                try
                {
                    ...
                    classNPOI.ExportDataToExcel(fileTeamplate, excel, 6, columnRemove, lstCell);
        ...
        public string ExportExcel_PhieuTraHang(Params_GetListHoaDon listParams)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                try
                {
                    ...
                    classNPOI.ExportDataToExcel(fileTeamplate, excel, 6, listParams.ColumnsHide, lstCell);
        ...
        [System.Web.Http.AcceptVerbs("GET", "POST")]
        public string ExportExcel_BaoGiaSuaChua(Params_GetListHoaDon listParams)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ClassBH_HoaDon classhoadon = new ClassBH_HoaDon(db);
                ...
                {
                    var lstAllHDs = classhoadon.GetListInvoice_Paging(listParams);
                    List<Excel_BaoGiaSuaChua> lst = new List<Excel_BaoGiaSuaChua>();
                    foreach (var item in lstAllHDs)
                    {
                        Excel_BaoGiaSuaChua DM = new Excel_BaoGiaSuaChua();
                        DM.MaHoaDon = item.MaHoaDon;
                        DM.NgayLapHoaDon = item.NgayLapHoaDon;
                        DM.MaPhieuTiepNhan = item.MaPhieuTiepNhan;
                    ...
                    string fileTeamplate = HttpContext.Current.Server.MapPath("~/Template/ExportExcel/" + "Teamplate_DanhSachBaoGiaSuaChua.xlsx");
                    fileSave = HttpContext.Current.Server.MapPath("~/Template/ExportExcel/" + "DanhSachBaoGiaSuaChua.xlsx");
                    fileSave = _classOFDCM.createFolder_Download(fileSave);
                    var valExcel1 = string.Empty;
                    if (listParams.NgayTaoHD_TuNgay == new DateTime(2016, 1, 1))
                    {
        ...
        public IHttpActionResult GetInforTheGiaTri_byID(Guid id)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                {
                    ClassBH_HoaDon classHoaDon = new ClassBH_HoaDon(db);
                    List<BH_HoaDonTheNapDTO> data = classHoaDon.GetInforTheGiaTri_byID(id);
                    return ActionTrueData(data);
                }
                catch (Exception e)
                {
        ...
        public string XuatFileThenNap(ModelHoaDonTheNap model)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                ...
                {
                    CookieStore.WriteLog("Export_XuatFileThenNap " + e.InnerException + e.Message);
                }
                return string.Empty;
            }
        }
        ...
        [HttpGet, HttpPost]
        public IHttpActionResult GetChiTiet_NhatKyGiaoDich_UsedBaoHanh(ParamNKyGDV param)
        {
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                try
                {
                    ...
                    if (param != null)
                    {
                        List<SP_NhatKySuDung_GoiDV> lst = classhoadon.GetChiTiet_NhatKyGiaoDich_UsedBaoHanh(param);
                        int count = 0, page = 0;
                        int currentPage = param.CurrentPage ?? 0, pageSize = param.PageSize ?? 20;
                        if (lst != null && lst.Count > 0)
                        {
            ...
            using (SsoftvnContext db = SystemDBContext.GetDBContext())
            {
                using (var trans = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        {
                            BH_HoaDon objU = db.BH_HoaDon.Find(objHoaDon.ID);
                            if (objU != null)
                            {
                                objU.MaHoaDon = sMaHoaDon;
                                objU.ID_NhanVien = objHoaDon.ID_NhanVien;
                                objU.ID_DoiTuong = objHoaDon.ID_DoiTuong;
                                objU.NgayLapHoaDon = objHoaDon.NgayLapHoaDon;
                                objU.TongChiPhi = objHoaDon.TongChiPhi; //mức nạp
                                objU.TongChietKhau = objHoaDon.TongChietKhau; //khuyến mại nạp thẻ
                                objU.TongTienHang = objHoaDon.TongTienHang; //tổng tiền nạp
                                objU.TongTienThue = objHoaDon.TongTienThue; //số dư sau nạp
                                objU.TongGiamGia = objHoaDon.TongGiamGia; //giảm giá cả hóa đơn
                                objU.PhaiThanhToan = objHoaDon.PhaiThanhToan; //phải thanh toán (= Tiền khách trả)
                                objU.TongThanhToan = objHoaDon.TongThanhToan;
                                objU.ID_DonVi = objHoaDon.ID_DonVi;
                                objU.NgaySua = DateTime.Now;
                                objU.NguoiSua = objHoaDon.NguoiSua;
                                objU.ChoThanhToan = objHoaDon.ChoThanhToan;
                                objU.DienGiai = objHoaDon.DienGiai;
                                objU.LoaiHoaDon = objHoaDon.LoaiHoaDon;
                            }
                        }
                        db.SaveChanges();
                        trans.Commit();

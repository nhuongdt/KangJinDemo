...
namespace banhang24.Areas.DanhMuc.Controllers
{
    public class BH_XuatHuyAPIController : BaseApiController
    {
        ...
        public IHttpActionResult getList_HangHoaXuatHuybyID_old(Guid ID_HoaDon, Guid ID_ChiNhanh)
        {
            SsoftvnContext db = SystemDBContext.GetDBContext();
            List<XH_HoaDon_ChiTietPRC> lst = new List<XH_HoaDon_ChiTietPRC>();
            List<XH_HoaDon_ChiTietPRC> lstreturn = new List<XH_HoaDon_ChiTietPRC>();
            List<SqlParameter> prm = new List<SqlParameter>();
            prm.Add(new SqlParameter("ID_HoaDon", ID_HoaDon));
            lst = db.Database.SqlQuery<XH_HoaDon_ChiTietPRC>("exec getList_HangHoaXuatHuybyID @ID_HoaDon", prm.ToArray()).ToList();
            ...
            List<XH_HoaDon_ChiTietPRC> lstCT = new List<XH_HoaDon_ChiTietPRC>();
            foreach (var item in lst)
            {
                List<SqlParameter> sqlPRM = new List<SqlParameter>();
                sqlPRM.Add(new SqlParameter("ID_ChiNhanh", ID_ChiNhanh));
                sqlPRM.Add(new SqlParameter("ID_DonViQuiDoi", item.ID_DonViQuiDoi));
                sqlPRM.Add(new SqlParameter("ID_LoHang", item.ID_LoHang != null ? item.ID_LoHang : Guid.NewGuid()));
                ...
                XH_HoaDon_ChiTietPRC DM = new XH_HoaDon_ChiTietPRC();
                DM.ID = item.ID;
                DM.ID_DonViQuiDoi = item.ID_DonViQuiDoi;
                DM.ID_HoaDon = item.ID_HoaDon;
                DM.ID_LoHang = item.ID_LoHang;
                DM.QuanLyTheoLoHang = item.QuanLyTheoLoHang;
                ...
                DM.TenHangHoaFull = item.TenHangHoaFull;
                DM.TenDonViTinh = item.TenDonViTinh;
                DM.ThuocTinh_GiaTri = item.ThuocTinh_GiaTri;
                DM.TenLoHang = item.TenLoHang;
                DM.TonKho = lstLH.Count() > 0 ? lstLH.FirstOrDefault().TonKho : item.TonKho;
                DM.SoLuong = item.SoLuong;
                DM.TrangThaiMoPhieu = item.TrangThaiMoPhieu;
                DM.SoLuongXuatHuy = item.SoLuongXuatHuy;
                DM.DonGia = item.DonGia;
                DM.GiamGia = item.GiamGia;
                DM.GiaVon = item.GiaVon;
                DM.GiaTriHuy = item.GiaTriHuy;
                DM.NgayLapHoaDon = item.NgayLapHoaDon;
                DM.ID_NhanVien = item.ID_NhanVien;
                DM.GhiChu = item.GhiChu;
                DM.SoThuTu = item.SoThuTu;
                lstCT.Add(DM);
            }

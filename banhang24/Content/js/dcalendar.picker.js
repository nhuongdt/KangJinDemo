...
+function ($) {
    ...
    var months = ['Tháng 1', 'Tháng 2', 'Tháng 3', 'Tháng 4', 'Tháng 5', 'Tháng 6', 'Tháng 7', 'Tháng 8', 'Tháng 9', 'Tháng 10', 'Tháng 11', 'Tháng 12'],
        short_months = ['Tháng 1', 'Tháng 2', 'Tháng 3', 'Tháng 4', 'Tháng 5', 'Tháng 6', 'Tháng 7', 'Tháng 8', 'Tháng 9', 'Tháng 10', 'Tháng 11', 'Tháng 12'],
        daysofweek = ['CN', 'Hai', 'Ba', 'Tư', 'Năm', 'Sáu', 'Bảy'],
        short_days = ['CN', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7'],
        ex_keys = [9, 112, 113, 114, 115, 116, 117, 118, 119, 120, 121, 122, 123],
        DCAL_DATA = 'dcalendar',
        DCalendar = function (elem, options, cb) {
			this.elem = $(elem);
	...
	DCalendar.prototype = {
		...
		selectDate: function () {
			...
			if (that.rangeFromEl) {
				if ($(that.rangeFromEl).data(DCAL_DATA) !== undefined) {
					var fromEl = $(that.rangeFromEl),
						fromData = fromEl.data(DCAL_DATA);
					cal = fromData.calendar;
					curr = cal.find('.calendar-dates:eq(0)'),
					lblMonth = cal.find('.calendar-curr-month'),
					lblDays = cal.find('.calendar-labels');

					fromData.getDays(fromData.selected, function (dates) {
						curr.html(dates);
					});

					calthat = that.calendar;
					currthat = calthat.find('.calendar-dates:eq(0)'),
						lblMonth = calthat.find('.calendar-curr-month'),
						lblDays = calthat.find('.calendar-labels');
			...
			if (that.rangeToEl) {
				if ($(that.rangeToEl).data(DCAL_DATA) !== undefined) {
					...
					toData.getDays(toData.selected, function (dates) {
						curr.html(dates);
					});
			...
			for(var i = 1; i <= 6; i++){
				...
				while(day <= days) {
					...
					if(i === 1 && dayOfWeek === 0){
						...
						if (that.inrangeDate(d)) week[dayOfWeek].addClass('in-range');
				...
				if (i === 1 || i > 4) {
					...
					// Last week
						...
						for (var a = 0; a <= 6; a++) {
							...
							//if (that.inrangeDate(nmDate)) week[a].addClass('in-range');
	...
	/* Sets current view based on user interaction (on arrows) */
        ...
        create: function () {
            ...
            for (var i = 0; i < daysofweek.length; i++) {
                header = header+ "<span>" + daysofweek[i] + "</span>";
            }
			var that = this,
                ...
                datesgrid = $('<section class="calendar-grid">'
                    + '<div class="calendar-labels">' + header + '</div><div class="calendar-date-holder"><section class="calendar-dates"></section></div></section>');
	...
	$.fn.dcalendar = function (opts, callback) {
		return $(this).each(function(index, elem){
			...
			if (!data) {
				$this.data(DCAL_DATA, (data = new DCalendar(this, options, callback)));
 			}

...
BEGIN
	...
	ROW_NUMBER() over (partition by tbl.ID_DonVi,
							tbl.ID_DonViQuiDoi,
							tbl.ID_LoHang,tbl.ID_ChiTietDinhLuong order by tbl.NgayDieuChinh desc) as RN
...
BEGIN
  set nocount on;
		...
		join BH_HoaDon hd on ct.ID_HoaDon = hd.ID	
		where hd.LoaiHoaDon in (5, 7, 8,35, 37, 38, 39, 40) and (ct.ChatLieu is null or ct.ChatLieu not in ('2'))
...
BEGIN
	...
	WHERE hd.ChoThanhToan = 0 and hd.LoaiHoaDon in (35,37,38,39,40,8,4,7,9,10,13,14,18) 
...
BEGIN
	...
	(
		...
		and hh.ID = @ID_HangHoa 
		and hd.ChoThanhToan = 0 
		and hd.LoaiHoaDon in (35,37,38,39,40,8,4,7,9,10,13,14,18) 			

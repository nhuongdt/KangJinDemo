namespace Model.Migrations
{
    ...
    public partial class UpdateSP_20190702 : DbMigration
    {
        public override void Up()
        {
            CreateStoredProcedure(name: "[dbo].[UpdateKiemKeTuHoaDon]", parametersAction: p => new
            {
    	...
    	TyLeChuyenDoi FLOAT, TonKho FLOAT, IDDonViQuiDoi UNIQUEIDENTIFIER, ID_LoHang UNIQUEIDENTIFIER, RN INT);
		...
		ROW_NUMBER() OVER (PARTITION BY hdctupdate.ID, hdctupdate.ID_LoHang ORDER BY hdupdate.NgayLapHoaDon) AS RN
    	...
    	hdupdate.ID_DonVi = cthdthemmoiupdate.ID_ChiNhanh and hdupdate.NgayLapHoaDon >= cthdthemmoiupdate.NgayLapHoaDon) as tbl where RN = 1;
  ...
  --  	--Update Kiem ke
        ...
        {
            DropStoredProcedure("[dbo].[UpdateKiemKeTuHoaDon]");
        }
    }
}
